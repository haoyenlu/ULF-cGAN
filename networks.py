...
class cGAN:
    ...
    def train(self,dataloader):
        summary(self.G,torch.zeros(2,self.seq_len,device=self.device),torch.zeros(2,dtype=torch.long,device=self.device))
        summary(self.D,torch.zeros(2,self.features,self.seq_len,device=self.device),torch.zeros(2,dtype=torch.long,device=self.device))
